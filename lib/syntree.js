...
Node.prototype.align_leaf_nodes = function() {
    ...
    // function find_leaf_y(node) {
    //     if (!node.has_children) {
    //         if (leafY === null) {
    //             leafY = node.y;  // Đặt leafY lần đầu tiên gặp một nút lá
    //         }
    //     } else {
	...
	function find_last_leaf_y(node) {
        if (!node.has_children) {
            leafY = node.y;  // Cập nhật leafY mỗi khi gặp một nút lá
        } else {
            for (let child = node.first; child != null; child = child.next) {
                find_last_leaf_y(child);  // Tiếp tục tìm nút lá cuối cùng
            }
        }
    }
    ...
    function set_leaf_y(node) {
        if (!node.has_children) {
            node.y = leafY;  // Đặt lại vị trí y cho các nút lá
        } else {
            for (let child = node.first; child != null; child = child.next) {
                set_leaf_y(child);
            }
    ...
    // Tìm giá trị y chung cho các nút lá
    find_last_leaf_y(this);
