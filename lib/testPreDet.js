...
const QA = {
  type: "QA",
  single_words: ["much", "many", "several", "few", "little"],
  word_group: [
    "plenty of",
    "a lot of",
    "lots of",
...
var array = [
  ...
  { name: "a great deal of", type: "QA" },
  { name: "garlic", type: "noun" },
  // { name: "long", type: "ADJ" },
  // { name: "black", type: "ADJ" },
  // { name: "table", type: "noun" },
];
...
function apRecongize(array_temp) {
  ...
  function subAP(array) {
    ...
    //mặc định DEG chỉ có 1 từ
    str += `[AP`;
    //DEG
    function degRecongize(array) {
      array.forEach((element) => {
        str += DEG.type.includes(element.type)
          ? `[${DEG.expend} ${element.name}]`
          : "";
      });
    }
    degRecongize(array);
